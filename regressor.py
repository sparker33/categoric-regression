...
class regressor:
    def __init__(self, data:pd.DataFrame, cardinal_dims:list, result_dim:int):
        self.data = data
    ...
    def get_Y_pred_Series(self) -> pd.Series:
        return pd.Series([p[0] for p in self.Y_pred])
    ...
    def generate_regression(self) -> None:
        ...
        self.Y = self.data.iloc[:, self.result_dim].values.reshape(-1, 1)  # -1 means that calculate the dimension of rows, but have 1 column
    ...
    def plot_regression(self, axis1:int = 0, axis2:int = 1) -> None:
        ...
        ax.scatter3D(self.X[:,axis1], self.X[:,axis2], self.Y, c=self.Y_pred, cmap='Greens')
